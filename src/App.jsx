      ...
      {tab==="analyze"&&(<>
        ...
        {result&&!loading&&!parseError&&(<div className="fadeUp" style={{marginTop:18}}>
          ...
          <Card style={{textAlign:"center",padding:"20px",position:"relative",overflow:"hidden"}}>
            ...
            {(result.confidence==="medium"||result.confidence==="low")&&result.confidenceNote&&(<p style={{fontSize:11,color:"#a0a0c0",lineHeight:1.5,marginTop:8,position:"relative",padding:"0 8px"}}>ℹ️ {result.confidenceNote}</p>)}
          ...
          return(<div ref={portionSectionRef} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"14px 16px",margin:"12px 0",transition:"all .3s ease"}}>
            {portionConfirmed?(<p style={{fontSize:12,color:T.accent,fontWeight:700,textAlign:"center",margin:0,animation:"fadeIn .3s ease"}}>✓ Great — accuracy confirmed</p>):(<>
              ...
              {portionFlow==="more"&&(<div style={{animation:"fadeIn .25s ease"}}>
                ...
                <button onClick={()=>setPortionFlow("initial")} style={{marginTop:8,background:"none",border:"none",color:T.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",padding:0}}>← Back</button>
              </div>)}
          ...
          <button onClick={()=>{setResult(null);setImage(null);setImgB64(null);setTextFood("");setPortionFlow("initial");setPortionConfirmed(false);setShowConfirmMsg(false);setParseError(false);}} className="ripple" style={{width:"100%",padding:"13px",borderRadius:14,border:`1px solid ${T.border}`,background:T.inputBg,color:T.text,cursor:"pointer",fontSize:14,fontFamily:"inherit",fontWeight:600,transition:"all .2s"}}>📸 Analyse Another</button>
