...
import { Camera, ClipboardList, BarChart3, User, Flame, Target, Zap, UtensilsCrossed, Brain, Droplets, Trophy, Heart, ArrowLeft, Upload, Search, Settings, LogOut, ChevronRight, Lock, Unlock, HelpCircle, Plus, Minus, X, Star, Activity, Scale, Ruler, Calendar, Sun, Moon } from "lucide-react";
...
const THEMES={
  dark:{bg:"#08080e",card:"#111118",border:"#1c1c2a",accent:"#00e5a0",accentDim:"#00e5a015",accentGlow:"#00e5a035",orange:"#ff6b35",orangeDim:"#ff6b3515",blue:"#4facfe",blueDim:"#4facfe15",purple:"#a78bfa",purpleDim:"#a78bfa15",text:"#eeeef5",muted:"#777799",danger:"#ff4757",inputBg:"#0e0e16",headerBg:"linear-gradient(180deg,rgba(14,14,24,0.95),rgba(12,12,19,0.9))",barBg:"#1a1a28",stepBg:"#0c0c13"},
  light:{bg:"#f5f5f8",card:"#ffffff",border:"#e2e2ea",accent:"#00c98a",accentDim:"#00c98a12",accentGlow:"#00c98a25",orange:"#e85d2c",orangeDim:"#e85d2c12",blue:"#3b8bdb",blueDim:"#3b8bdb12",purple:"#8b6fc0",purpleDim:"#8b6fc012",text:"#1a1a2e",muted:"#6b6b80",danger:"#e04050",inputBg:"#f0f0f5",headerBg:"linear-gradient(180deg,rgba(245,245,248,0.95),rgba(240,240,245,0.9))",barBg:"#e0e0e8",stepBg:"#f0f0f5"}
};
...
const Card=({children,style={}})=>(<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:20,padding:"18px 20px",marginBottom:14,boxShadow:`0 4px 16px ${T.bg}15`,backdropFilter:"blur(10px)",...style}}>{children}</div>);
...
function StepBody({p,setP,onNext,onBack}){const[helpType,setHelpType]=useState(null);const ok=p.height&&p.weight&&parseFloat(p.height)>0&&parseFloat(p.weight)>0;return(<div className="slideIn" style={{minHeight:"100vh",background:T.bg}}><div style={{padding:"20px 22px 16px",background:T.stepBg,borderBottom:`1px solid ${T.border}`}}><button onClick={onBack} style={{background:"none",border:"none",color:T.muted,fontSize:22,cursor:"pointer",marginBottom:14}}><ArrowLeft size={20}/></button><ProgressDots total={4} current={1}/><h2 style={{fontSize:22,fontWeight:800,marginBottom:4,color:T.text}}>Body Measurements</h2><p style={{color:T.muted,fontSize:13}}>Step 2 of 4 — Used to calculate your BMR</p></div><div style={{padding:"22px 20px 40px",maxWidth:480,margin:"0 auto"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}><p style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:".7px",fontWeight:700}}>Height</p><button onClick={()=>setHelpType("height")} style={{background:T.accentDim,border:`1px solid ${T.accent}30`,borderRadius:8,padding:"3px 10px",fontSize:11,color:T.accent,fontWeight:600,cursor:"pointer"}}>📏 How to measure</button></div><div style={{display:"flex",alignItems:"center",background:T.inputBg,border:`1.5px solid ${T.border}`,borderRadius:12,overflow:"hidden",marginBottom:15}}><input type="number" value={p.height} onChange={e=>setP(x=>({...x,height:e.target.value}))} placeholder="Enter your height" style={{flex:1,padding:"13px 16px",background:"transparent",border:"none",color:T.text,fontSize:16,fontWeight:700,outline:"none",fontFamily:"inherit"}}/><span style={{padding:"0 16px",color:T.muted,fontSize:13,fontWeight:600}}>cm</span></div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}><p style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:".7px",fontWeight:700}}>Current Weight</p><button onClick={()=>setHelpType("weight")} style={{background:T.accentDim,border:`1px solid ${T.accent}30`,borderRadius:8,padding:"3px 10px",fontSize:11,color:T.accent,fontWeight:600,cursor:"pointer"}}>⚖️ How to measure</button></div><div style={{display:"flex",alignItems:"center",background:T.inputBg,border:`1.5px solid ${T.border}`,borderRadius:12,overflow:"hidden",marginBottom:15}}><input type="number" value={p.weight} onChange={e=>setP(x=>({...x,weight:e.target.value}))} placeholder="Enter your weight" style={{flex:1,padding:"13px 16px",background:"transparent",border:"none",color:T.text,fontSize:16,fontWeight:700,outline:"none",fontFamily:"inherit"}}/><span style={{padding:"0 16px",color:T.muted,fontSize:13,fontWeight:600}}>kg</span></div><NumInput label="Target Weight (optional)" value={p.targetWeight} onChange={v=>setP(x=>({...x,targetWeight:v}))} unit="kg" placeholder="Enter target weight"/><Btn onClick={onNext} disabled={!ok}>Continue →</Btn></div>{helpType&&<MeasureHelp type={helpType} onClose={()=>setHelpType(null)}/>}</div>);}

function StepActivity({p,setP,onNext,onBack}){const opts=[{k:"sedentary",l:"Sedentary",d:"Desk job, little or no exercise",i:"🛋️"},{k:"light",l:"Lightly Active",d:"Walk sometimes, light exercise 1–3x/week",i:"🚶"},{k:"moderate",l:"Moderately Active",d:"Exercise 3–5x per week",i:"🏃"},{k:"active",l:"Very Active",d:"Hard training 6–7x/week",i:"🏋️"},{k:"very_active",l:"Extremely Active",d:"Physical job + daily training",i:"⚡"}];return(<div className="slideIn" style={{minHeight:"100vh",background:T.bg}}><div style={{padding:"20px 22px 16px",background:T.stepBg,borderBottom:`1px solid ${T.border}`}}><button onClick={onBack} style={{background:"none",border:"none",color:T.muted,fontSize:22,cursor:"pointer",marginBottom:14}}><ArrowLeft size={20}/></button><ProgressDots total={4} current={2}/><h2 style={{fontSize:22,fontWeight:800,marginBottom:4,color:T.text}}>Activity Level</h2><p style={{color:T.muted,fontSize:13}}>Step 3 of 4 — How active are you?</p></div><div style={{padding:"20px 20px 40px",maxWidth:480,margin:"0 auto"}}>{opts.map(o=>(<div key={o.k} onClick={()=>setP(x=>({...x,activity:o.k}))} style={{display:"flex",alignItems:"center",gap:13,padding:"13px 15px",marginBottom:9,background:p.activity===o.k?T.accentDim:T.inputBg,border:`1.5px solid ${p.activity===o.k?T.accent:T.border}`,borderRadius:14,cursor:"pointer",transition:"all .18s"}}><span style={{fontSize:24}}>{o.i}</span><div style={{flex:1}}><p style={{fontSize:14,fontWeight:700,color:p.activity===o.k?T.accent:T.text,marginBottom:2}}>{o.l}</p><p style={{fontSize:12,color:T.muted}}>{o.d}</p></div>{p.activity===o.k&&<span style={{color:T.accent,fontSize:18}}>✓</span>}</div>))}<Btn onClick={onNext} disabled={!p.activity} style={{marginTop:8}}>Continue →</Btn></div></div>);}

function StepGoal({p,setP,onNext,onBack}){const goals=[{k:"lose_fast",l:"Lose Weight Fast",d:"-750 kcal/day · ~0.75 kg/week",i:"🔥"},{k:"lose",l:"Lose Weight",d:"-500 kcal/day · ~0.5 kg/week",i:"📉",best:true},{k:"lose_slow",l:"Lose Gradually",d:"-250 kcal/day · ~0.25 kg/week",i:"🌱"},{k:"maintain",l:"Maintain Weight",d:"Eat at maintenance calories",i:"⚖️"},{k:"gain",l:"Build Muscle",d:"+300 kcal/day · lean bulk",i:"💪"}];return(<div className="slideIn" style={{minHeight:"100vh",background:T.bg}}><div style={{padding:"20px 22px 16px",background:T.stepBg,borderBottom:`1px solid ${T.border}`}}><button onClick={onBack} style={{background:"none",border:"none",color:T.muted,fontSize:22,cursor:"pointer",marginBottom:14}}><ArrowLeft size={20}/></button><ProgressDots total={4} current={3}/><h2 style={{fontSize:22,fontWeight:800,marginBottom:4,color:T.text}}>Your Goal</h2><p style={{color:T.muted,fontSize:13}}>Step 4 of 4 — Sets your daily calorie target</p></div><div style={{padding:"20px 20px 40px",maxWidth:480,margin:"0 auto"}}>{goals.map(g=>(<div key={g.k} onClick={()=>setP(x=>({...x,goal:g.k}))} style={{display:"flex",alignItems:"center",gap:13,padding:"13px 15px",marginBottom:9,background:p.goal===g.k?T.accentDim:T.inputBg,border:`1.5px solid ${p.goal===g.k?T.accent:T.border}`,borderRadius:14,cursor:"pointer",transition:"all .18s"}}><span style={{fontSize:24}}>{g.i}</span><div style={{flex:1}}><p style={{fontSize:14,fontWeight:700,color:p.goal===g.k?T.accent:T.text,marginBottom:2}}>{g.l}{g.best&&<span style={{fontSize:9,background:T.accent,color:"#000",borderRadius:5,padding:"2px 6px",marginLeft:7,fontWeight:800}}>BEST</span>}</p><p style={{fontSize:12,color:T.muted}}>{g.d}</p></div>{p.goal===g.k&&<span style={{color:T.accent,fontSize:18}}>✓</span>}</div>))}<Btn onClick={onNext} disabled={!p.goal} style={{marginTop:8}}>Calculate My Plan →</Btn></div></div>);}
...
function TrackerApp({profile,goal,uid,onEditProfile,onSignOut,theme,toggleTheme}){
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"'DM Sans',sans-serif",color:T.text,transition:"background .3s ease, color .3s ease"}}>
    ...
    <div style={{padding:"14px 18px 0",background:T.headerBg,borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,zIndex:10,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)"}}>
    ...
    <div style={{padding:"16px 16px 80px",maxWidth:480,margin:"0 auto"}}>
      {tab==="analyze"&&(<>
        ...
        {!image&&(<input style={{width:"100%",padding:"14px 16px",background:T.inputBg,border:`1.5px solid ${T.border}`,borderRadius:14,color:T.text,fontSize:14,outline:"none",marginBottom:12,fontFamily:"inherit",transition:"border-color .2s",boxShadow:"inset 0 2px 4px rgba(0,0,0,0.2)"}} placeholder={placeholders[placeholderIdx]} value={textFood} onChange={e=>setTextFood(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()} onFocus={e=>e.target.style.borderColor=T.accent+"60"} onBlur={e=>e.target.style.borderColor=T.border}/>)}
        ...
        {error&&<div style={{background:`${T.danger}08`,border:`1px solid ${T.danger}30`,borderRadius:14,padding:"13px 16px",marginTop:14,color:T.danger,fontSize:13,boxShadow:`0 4px 16px ${T.danger}10`}}>⚠️ {error}</div>}
        ...
        {result&&!loading&&(<div className="fadeUp" style={{marginTop:18}}>
          ...
          {imgB64&&!result._adjusted&&(<div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:T.inputBg,borderRadius:12,marginBottom:14,border:`1px solid ${T.border}`}}>
          ...
          {showPortionAdjust&&(<div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
            ...
            }} style={{padding:"8px 14px",borderRadius:10,border:`1.5px solid ${portionMultiplier===mult?T.accent:T.border}`,background:portionMultiplier===mult?T.accentDim:T.inputBg,color:portionMultiplier===mult?T.accent:T.text,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all .2s"}}>{label}</button>))}
          </div>)}
          ...
          <button onClick={()=>{setResult(null);setImage(null);setImgB64(null);setTextFood("");}} className="ripple" style={{width:"100%",padding:"13px",borderRadius:14,border:`1px solid ${T.border}`,background:T.inputBg,color:T.text,cursor:"pointer",fontSize:14,fontFamily:"inherit",fontWeight:600,transition:"all .2s"}}>📸 Analyse Another</button>
      ...
      {tab==="me"&&(<>
        ...
        {/* Actions */}
        <button onClick={onEditProfile} className="ripple" style={{width:"100%",padding:"14px",borderRadius:14,border:`1px solid ${T.border}`,background:T.inputBg,color:T.text,fontWeight:600,cursor:"pointer",fontFamily:"inherit",fontSize:14,marginBottom:10,transition:"all .2s",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.03)"}}>✏️ Edit Profile</button>
