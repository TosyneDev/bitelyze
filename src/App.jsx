...
const saveProfile=async(uid,profile)=>{await setDoc(doc(db,"users",uid,"data","profile"),profile,{merge:true});};
const loadProfile=async(uid)=>{const snap=await getDoc(doc(db,"users",uid,"data","profile"));return snap.exists()?snap.data():null;};
const saveTodayData=async(uid,data)=>{const today=new Date().toISOString().split("T")[0];await setDoc(doc(db,"users",uid,"days",today),data,{merge:true});};
const loadTodayData=async(uid)=>{const today=new Date().toISOString().split("T")[0];const snap=await getDoc(doc(db,"users",uid,"days",today));return snap.exists()?snap.data():null;};
const saveStats=async(uid,stats)=>{await setDoc(doc(db,"users",uid,"data","stats"),stats,{merge:true});};
const loadStats=async(uid)=>{const snap=await getDoc(doc(db,"users",uid,"data","stats"));return snap.exists()?snap.data():{streak:1,totalMeals:0,waterGoalHits:0,daysUnderGoal:0,earlyBreakfast:0};};
...
function Pulse(){return(<div style={{textAlign:"center",padding:"28px 0"}}><div style={{display:"flex",justifyContent:"center",gap:6,marginBottom:12}}>{[0,1,2].map(i=>(<div key={i} style={{width:9,height:9,borderRadius:"50%",background:T.accent,animation:`pulse .8s ease-in-out ${i*.2}s infinite`,display:"inline-block"}}/>))}</div><p style={{color:T.muted,fontSize:13}}>Analyzing your food with AI…</p></div>);}
...
function AuthScreen(){
  ...
  const handleEmail=async()=>{if(!email||!pass)return setError("Please fill in all fields");if(mode==="signup"&&!name)return setError("Please enter your name");setLoading(true);setError("");try{if(mode==="signup"){const cred=await createUserWithEmailAndPassword(auth,email,pass);try{await Promise.race([saveProfile(cred.user.uid,{name}),new Promise(r=>setTimeout(r,5000))]);}catch(e){console.log("Profile save error:",e);}}else{await signInWithEmailAndPassword(auth,email,pass);}}catch(e){setError(e.message.replace("Firebase: ","").replace(/\(auth.*\)/,""));}finally{setLoading(false);}};
  const handleForgotPassword=async()=>{if(!email)return setError("Enter your email address first");setLoading(true);setError("");try{await sendPasswordResetEmail(auth,email);setResetSent(true);}catch(e){setError(e.message.replace("Firebase: ","").replace(/\(auth.*\)/,""));}finally{setLoading(false);}};
  const handleGoogle=async()=>{setLoading(true);setError("");try{await signInWithPopup(auth,googleProvider);}catch(e){setError(e.message.replace("Firebase: ","").replace(/\(auth.*\)/,""));}finally{setLoading(false);}};
...
function TrackerApp({profile,goal,uid,onEditProfile,onSignOut}){
  ...
  const ringR=44;const ringC=2*Math.PI*ringR;

  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"'DM Sans',sans-serif",color:T.text}}>
    {/* ── Premium Header ── */}
    <div style={{padding:"14px 18px 12px",background:"linear-gradient(180deg,#0e0e18,#0c0c13)",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10,backdropFilter:"blur(16px)"}}>
      <div style={{width:40,height:40,borderRadius:14,background:`linear-gradient(135deg,${T.accent},#00b87a)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,boxShadow:`0 4px 16px ${T.accentGlow}`}}>🍽️</div>
      <div style={{flex:1}}><p style={{fontSize:16,fontWeight:900,letterSpacing:"-0.02em"}}>Bitelyze</p><p style={{fontSize:10,color:T.muted,fontWeight:500}}>{TODAY}{saving&&" · saving…"}</p></div>
      <div style={{display:"flex",alignItems:"center",gap:6,background:"linear-gradient(135deg,#2a1a0a,#1a0f05)",border:`1px solid ${T.orange}35`,borderRadius:24,padding:"5px 12px",boxShadow:`0 0 16px ${T.orange}15`}}><span style={{fontSize:14}}>🔥</span><span style={{fontSize:13,fontWeight:900,color:T.orange}}>{stats.streak}d</span></div>
      <button onClick={onSignOut} style={{background:"#16162a",border:`1px solid ${T.border}`,color:T.muted,borderRadius:10,padding:"7px 12px",fontSize:11,cursor:"pointer",fontFamily:"inherit",fontWeight:600,transition:"all .2s"}}>↩ Out</button>
    </div>

    {/* ── Calorie Ring Summary ── */}
    <div style={{padding:"20px 18px 16px",background:"linear-gradient(180deg,#0c0c15,#08080e)"}}>
      <div style={{display:"flex",alignItems:"center",gap:20}}>
        <div style={{position:"relative",flexShrink:0}} className="ring-glow">
          <svg width="100" height="100" viewBox="0 0 100 100">
            <defs><linearGradient id="rg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor={T.accent}/><stop offset="100%" stopColor="#00b87a"/></linearGradient></defs>
            <circle cx="50" cy="50" r={ringR} fill="none" stroke="#1a1a28" strokeWidth="8"/>
            <circle cx="50" cy="50" r={ringR} fill="none" stroke="url(#rg)" strokeWidth="8" strokeDasharray={ringC} strokeDashoffset={ringC*(1-Math.min(consumed/goal,1))} strokeLinecap="round" transform="rotate(-90 50 50)" style={{transition:"stroke-dashoffset 1s cubic-bezier(0.16,1,0.3,1)"}}/>
          </svg>
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
            <span style={{fontSize:22,fontWeight:900,color:T.accent,lineHeight:1}}>{pct}%</span>
            <span style={{fontSize:8,color:T.muted,textTransform:"uppercase",letterSpacing:".5px",marginTop:2}}>eaten</span>
          </div>
        </div>
        <div style={{flex:1,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
          {[["Goal",goal,T.accent],["Eaten",consumed,T.orange],[rem<0?"Over":"Left",Math.abs(rem),rem<0?T.danger:T.blue]].map(([l,v,c])=>(<div key={l} style={{background:`${c}08`,border:`1px solid ${c}20`,borderRadius:14,padding:"10px 8px",textAlign:"center"}}>
            <span style={{fontSize:18,fontWeight:900,color:c,display:"block",letterSpacing:"-0.02em"}}>{v}</span>
            <span style={{fontSize:9,color:T.muted,textTransform:"uppercase",letterSpacing:".4px",fontWeight:600}}>{l}</span>
          </div>))}
        </div>
      </div>
      {/* Thin gradient progress bar */}
      <div style={{height:5,borderRadius:99,background:"#1a1a28",marginTop:14,overflow:"hidden",boxShadow:"inset 0 1px 3px rgba(0,0,0,0.4)"}}><div style={{height:"100%",width:`${Math.min((consumed/goal)*100,100)}%`,background:`linear-gradient(90deg,${T.accent},${T.orange})`,borderRadius:99,transition:"width .8s cubic-bezier(0.16,1,0.3,1)",boxShadow:`0 0 10px ${T.accent}40`}}/></div>
    </div>
    ...
    <div style={{margin:"0 16px",padding:"16px 18px",background:`linear-gradient(145deg,${cm.color}0a,${cm.color}05)`,border:`1px solid ${cm.color}25`,borderRadius:18,marginTop:12,boxShadow:`0 4px 24px ${cm.color}08`}}>
      <div style={{display:"flex",gap:12,alignItems:"flex-start",marginBottom:10}}>
        <div style={{width:36,height:36,borderRadius:12,background:`${cm.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0,border:`1px solid ${cm.color}25`}}>🧠</div>
      ...
      <div style={{padding:"10px 14px",background:"rgba(255,255,255,0.03)",borderRadius:12,borderLeft:`3px solid ${T.orange}`,boxShadow:"inset 0 1px 0 rgba(255,255,255,0.02)"}}><p style={{fontSize:12,color:"#d0c090",lineHeight:1.55}}>{coachTips[tipIdx]}</p></div>
    </div>

    {/* ── Premium Tab Bar ── */}
    <div style={{display:"flex",margin:"14px 16px 0",background:"#0e0e18",borderRadius:16,padding:4,border:`1px solid ${T.border}`}}>
      {[["analyze","📸","Analyze"],["log","📋","Log"],["progress","📊","Progress"],["me","👤","Me"]].map(([k,ic,lb])=>(<button key={k} onClick={()=>setTab(k)} style={{flex:1,padding:"10px 4px",border:"none",background:tab===k?`linear-gradient(135deg,${T.accent}18,${T.accent}08)`:"transparent",color:tab===k?T.accent:T.muted,fontSize:11,fontWeight:700,cursor:"pointer",borderRadius:12,transition:"all .25s cubic-bezier(0.16,1,0.3,1)",fontFamily:"inherit",boxShadow:tab===k?`0 0 12px ${T.accent}15,inset 0 1px 0 rgba(255,255,255,0.04)`:"none"}}>{ic} {lb}</button>))}
    </div>

    <div style={{padding:"16px 16px 80px",maxWidth:480,margin:"0 auto"}}>
      {tab==="analyze"&&(<>
        {!image?(<div className="upload" onClick={()=>fileRef.current.click()} style={{border:`2px dashed ${T.border}`,borderRadius:20,padding:"32px 20px",textAlign:"center",cursor:"pointer",background:"linear-gradient(145deg,#0e0e18,#0a0a14)",marginBottom:12,transition:"all .25s",boxShadow:"0 4px 20px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.03)"}}><div style={{width:56,height:56,borderRadius:18,background:`${T.accent}12`,border:`1px solid ${T.accent}25`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,margin:"0 auto 14px"}}>📷</div><p style={{fontWeight:800,fontSize:15,marginBottom:4,letterSpacing:"-0.01em"}}>Snap or upload your meal</p><p style={{fontSize:12,color:T.muted}}>JPG or PNG • or type food name below</p><input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>handleFile(e.target.files[0])}/></div>):(<div style={{position:"relative",marginBottom:12}}><img src={image} alt="food" style={{width:"100%",borderRadius:18,maxHeight:240,objectFit:"cover",border:`1px solid ${T.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.3)"}}/><button onClick={()=>{setImage(null);setImgB64(null);setResult(null);}} style={{position:"absolute",top:12,right:12,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(8px)",border:`1px solid ${T.border}`,color:"#fff",borderRadius:10,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:600}}>✕</button></div>)}
        {!image&&(<input style={{width:"100%",padding:"14px 16px",background:"#0e0e18",border:`1.5px solid ${T.border}`,borderRadius:14,color:T.text,fontSize:14,outline:"none",marginBottom:12,fontFamily:"inherit",transition:"border-color .2s",boxShadow:"inset 0 2px 4px rgba(0,0,0,0.2)"}} placeholder="Or type food name, e.g. 'Grilled chicken with rice'…" value={textFood} onChange={e=>setTextFood(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()} onFocus={e=>e.target.style.borderColor=T.accent+"60"} onBlur={e=>e.target.style.borderColor=T.border}/>)}
        ...
        {result&&!loading&&(<div className="fadeUp" style={{marginTop:18}}>
          {/* Result Hero Card */}
          <div style={{background:`linear-gradient(145deg,#0f1a14,#0a1410)`,border:`1.5px solid ${T.accent}30`,borderRadius:22,padding:"22px 20px",marginBottom:14,boxShadow:`0 8px 40px ${T.accent}10, inset 0 1px 0 rgba(255,255,255,0.04)`,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,position:"relative"}}>
              <div><p style={{fontSize:19,fontWeight:900,marginBottom:4,letterSpacing:"-0.02em"}}>{result.foodName}</p><p style={{fontSize:12,color:T.muted,fontWeight:500}}>{result.servingSize}</p></div>
              <div style={{textAlign:"center",background:`${hc}15`,border:`1.5px solid ${hc}30`,borderRadius:14,padding:"8px 14px",boxShadow:`0 0 16px ${hc}10`}}>
                <span style={{fontSize:20,fontWeight:900,color:hc,display:"block",lineHeight:1}}>{result.healthScore}</span><span style={{fontSize:8,color:T.muted,textTransform:"uppercase",letterSpacing:".5px"}}>/10 Health</span>
              </div>
            </div>
            <div style={{textAlign:"center",padding:"16px 0 8px",borderTop:`1px solid ${T.border}`,position:"relative"}}>
              <span style={{fontSize:56,fontWeight:900,color:T.accent,letterSpacing:"-0.03em",textShadow:`0 0 40px ${T.accentGlow}`}}>{result.totalCalories}</span><span style={{fontSize:14,color:T.muted,marginLeft:6,fontWeight:600}}>kcal</span>
            </div>
          </div>
          {/* Macros Card */}
          <Card><CardTitle icon="⚗️">Macronutrients</CardTitle><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:14}}>{[["Protein",result.protein,T.blue],["Carbs",result.carbs,T.orange],["Fat",result.fat,"#ff6b9d"],["Fiber",result.fiber,T.accent]].map(([l,v,c])=>(<div key={l} style={{textAlign:"center",background:`${c}08`,borderRadius:12,padding:"10px 4px",border:`1px solid ${c}15`}}><span style={{fontSize:20,fontWeight:900,color:c,display:"block",lineHeight:1,marginBottom:4}}>{v}g</span><span style={{fontSize:10,color:T.muted,fontWeight:500}}>{l}</span></div>))}</div><MacroBar label="Protein" val={result.protein} max={150} color={T.blue}/><MacroBar label="Carbs" val={result.carbs} max={300} color={T.orange}/><MacroBar label="Fat" val={result.fat} max={80} color="#ff6b9d"/><MacroBar label="Fiber" val={result.fiber} max={40} color={T.accent}/></Card>
          {/* Suggestions */}
          <Card><CardTitle icon="💡">Coach Suggestions</CardTitle>{result.suggestions?.map((s,i)=>(<div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:i<result.suggestions.length-1?`1px solid ${T.border}`:"none"}}><span style={{fontSize:17,flexShrink:0}}>{s.icon}</span><span style={{fontSize:13,lineHeight:1.6,color:"#c8c8e0"}}>{s.text}</span></div>))}</Card>
          {/* Exercises */}
          <Card><CardTitle icon="🏋️">Burn It Off</CardTitle>{result.exercises?.map((e,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:i<result.exercises.length-1?`1px solid ${T.border}`:"none"}}><div><p style={{fontSize:13,fontWeight:700,marginBottom:2}}>{e.name}</p><p style={{fontSize:11,color:T.muted}}>{e.duration}</p></div><span style={{fontSize:12,color:T.orange,fontWeight:800,background:T.orangeDim,padding:"4px 10px",borderRadius:8}}>-{e.calories} kcal</span></div>))}</Card>
          <div style={{display:"flex",gap:10}}><Btn onClick={()=>logMeal()} style={{flex:1,width:"auto"}}>✅ Log Meal</Btn><button onClick={()=>{setResult(null);setImage(null);setImgB64(null);setTextFood("");}} style={{padding:"14px 16px",borderRadius:14,border:`1px solid ${T.border}`,background:"#16162a",color:T.muted,cursor:"pointer",fontSize:14,fontFamily:"inherit",transition:"all .2s"}}>✕</button></div>
        </div>)}
        ...
        <Card style={{background:"linear-gradient(145deg,#18120a,#120e06)",borderColor:`${T.orange}25`,position:"relative",overflow:"hidden",padding:"22px 20px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:20,position:"relative"}}>
            <div style={{textAlign:"center",background:`${T.orange}0a`,borderRadius:20,padding:"14px 18px",border:`1px solid ${T.orange}20`}}>
              <p style={{fontSize:48,fontWeight:900,color:T.orange,lineHeight:1,textShadow:`0 0 30px ${T.orange}30`}}>{stats.streak}</p>
      ...
      {tab==="me"&&(<>
        ...
        {/* Stats */}
        <Card><CardTitle icon="📈">My Stats</CardTitle><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6}}>{[["Total Meals",allHistory.length,T.accent,"🍽️"],["Streak",`${stats.streak}d`,T.orange,"🔥"],["Badges",`${earned.length}/${BADGES.length}`,T.purple,"🏆"],["Water",`${water}/8`,T.blue,"💧"]].map(([k,v,c,ic])=>(<div key={k} style={{background:`${c}08`,border:`1px solid ${c}15`,borderRadius:14,padding:"14px 12px",textAlign:"center"}}><span style={{fontSize:18,display:"block",marginBottom:4}}>{ic}</span><span style={{fontSize:18,fontWeight:900,color:c,display:"block",lineHeight:1}}>{v}</span><span style={{fontSize:10,color:T.muted,marginTop:4,display:"block",fontWeight:500}}>{k}</span></div>))}</div></Card>
        {/* Actions */}
...
export default function App(){
  ...
  const saveAndContinue=async()=>{
    const savePromise=(async()=>{try{if(authUser)await saveProfile(authUser.uid,profile);}catch(e){console.log("Profile save error:",e);}})();
